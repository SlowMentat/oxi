                ...
                else if (ase instanceof AccessDeniedException)
                {
                    ...
                    if (authenticationTrustResolver.isAnonymous(SecurityContextHolder.getContext().getAuthentication()))
                    {
                        ...
                        if ("XMLHttpRequest".equals(ajaxHeader))
                        {
							...
							//resp.setHeader("Status Code", Integer.toString(this.customSessionExpiredErrorCode));
                            resp.setStatus(this.customSessionExpiredErrorCode);
							resp.setHeader("Location", this.redirectLocation);
							for (String header : resp.getHeaderNames()){
								logger.debug(header + ":  " + resp.getHeaders(header));
							}
                            //resp.sendError(this.customSessionExpiredErrorCode);
							//resp.sendRedirect(this.redirectLocation);
                        }
                        else
                        {
